...
function energy_vs_fitness_peak1(data_vp1_peak1,H_vp1_peak1)
...
%not sure what to assign as neither P nor T exist at this position
...
for i = 1:numSeqMsaToTest
    ...
    if i == 2
        EnergySeq(i) = EnergySeq(i) + no_conserved_sites_in_indices_diff * min(diag(H)); %because S is consensus in MSA
    else
        EnergySeq(i) = EnergySeq(i) - no_conserved_sites_in_indices_diff * min(diag(H));
    end
...
vp1_mahoney_W170R_T177S([170 177]) = 'RK';

vp1_mahoney_V160I_W170R_T177S = vp1_mahoney;
vp1_mahoney_V160I_W170R_T177S([160 170 177]) = 'IRK';
...
for i = 1:numSeqMsaToTest
    ...
    if i==8
        EnergySeq3(i) = EnergySeq3(i) - no_conserved_sites_in_indices_diff*min(diag(H)); %because in MSA, consensus is L
    elseif i == 2 || i == 5
        EnergySeq3(i) = EnergySeq3(i) + no_conserved_sites_in_indices_diff*min(diag(H)); %because in MSA, consensus is A and S respectively
    end
...
text(1,2.6,'$$\rho_s = -0.83$$','interpreter','latex','FontSize',12)
% % text(1.5,2.3,'$$p \;= 0.042$$','interpreter','latex','FontSize',12)
text(1,2.2,'$$p \;= 4.5\times10^{-8}$$','interpreter','latex','FontSize',12)
